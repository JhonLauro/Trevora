...
@Entity
@Table(name = "service_record_items")
public class ServiceRecordItem {
     ...
     *
     * <p>Hydration is {@code ServiceRecordItemReader}'s job and happens on every
     * read path, so this is empty only when the item genuinely has no lines.
     */
    @Transient
    private List<ServiceRecordLineEntry> lineEntries = List.of();
    ...
    public List<ServiceRecordLineEntry> getLineEntries() {
        return lineEntries;
    }

    public void setLineEntries(List<ServiceRecordLineEntry> lineEntries) {
